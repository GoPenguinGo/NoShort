...
def simulate_cohorts_mix_type(
    ...
    if need_pi == 'True':
        ...
        pi = 0
    invest_mat = np.zeros((12, Nconstraint, Nc), dtype=np.int8)
...
def simulate_cohorts_mix_impulse(
        biasvec: np.ndarray,
        dZ: np.ndarray,
        dZ_SI: np.ndarray,
        Nt: int,
    ...
    invest_mat = np.zeros((12, Nconstraint, Nc), dtype=np.int8)
    # equilibrium terms:
    dR = np.zeros((3, Nr))  # stores stock returns
    r = np.zeros((3, Nr))  # interest rate
    theta = np.zeros((3, Nr))  # market price of risk
    mu_S = np.zeros((3, Nr))
    sigma_S = np.zeros((3, Nr))
    beta = np.zeros((3, Nr))
    parti = np.ones((3, Nr), dtype=np.float16)  # participation rate
    entry_mat = np.ones((3, Nr), dtype=np.float16)
    exit_mat = np.ones((3, Nr), dtype=np.float16)
    parti_wealth_group = np.zeros((3, Nr, 4), dtype=np.float16)
    parti_age_group = np.zeros((3, Nr, 4), dtype=np.float16)
