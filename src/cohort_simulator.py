...
def simulate_cohorts_mix_type(
    ...
    if need_f == 'True':
        f_c = np.zeros((Nt, Ntype, Nconstraint, Nc))    # evolution of cohort consumption share
        f_w = np.zeros((Nt, Ntype, Nconstraint, Nc))   # evolution of cohort wealth share
    else:
        f_c = f_w = 0

    if need_Delta == 'True':
        Delta = np.zeros((Nt, Ntype, Nconstraint, Nc))  # stores bias in beliefs
    else:
        Delta = 0

    if need_pi == 'True':
        pi = np.zeros((Nt, Ntype, Nconstraint, Nc))  # portfolio choices
    else:
        pi = 0
    ...
    for i in tqdm(range(Nt)):
        ...
        if i == 0:
            ...
            w_ist = np.append(w_ist[:, :, 1:] * adjust_scale, append_init * w_t * tax, axis=2)
            w_indiv_ist = np.append(w_indiv_ist[:, :, 1:] * adjust_scale, append_init * w_t * tax / nu, axis=2)
...
def simulate_cohorts_mean_vola(
    ...
    dR_matrix = [np.mean(dR), np.std(dR)]
    theta_matrix = [np.mean(theta), np.std(theta)]
    r_matrix = [np.mean(r), np.std(r)]
    mu_S_matrix = [np.mean(mu_S), np.std(mu_S)]
    sigma_S_matrix = [np.mean(sigma_S), np.std(sigma_S)]
    beta_matrix = [np.mean(beta), np.std(beta)]

    Delta_bar_parti_matrix = [np.mean(Delta_bar_parti), np.std(Delta_bar_parti)]
    Delta_tilde_parti_matrix = [np.mean(Delta_tilde_parti), np.std(Delta_tilde_parti)]
