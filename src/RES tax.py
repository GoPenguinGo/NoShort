...
def BuildUpCohortsMAIN(dZt, Nt, dt, rho, nu, Vbar, mu_Y, sigma_Y, bet, That):
    ...
    for i in range(1, Nt):
        Part = IntVec * np.exp(-(rho + 0.5 * Delta_s_t * Delta_s_t) * dt + Delta_s_t * dZt[i - 1])
        if i == 1:
            Xt[i] = Part
...
def SimCohortsMAIN(biasvec, dZt, Nt, tau, IntVec, Delta_s_t, dt, rho, nu, Vbar, mu_Y, sigma_Y, sigma_S, bet, That,
                   Npre):
    ...
    for i in range(Nt):
        ...
        if i == 0:
            ...
            dR[i] = (mu_S[i - 1] - r_t[i - 1] + rho + mu_Y - sigma_Y ** 2 + nu * (1 - bet)) * dt + sigma_S * \
                    dZt[i]
        ...
        Et[i] = np.sum(f * (Vbar / (1 + (Vbar / sigma_Y ** 2) * dt * RevNt)) * (1 / sigma_Y))
        Vt[i] = (np.sum(f * Delta_s_t ** 2) - Deltabar2[i] ** 2) * sigma_Y

        fst[i] = f[Nt - i - 1]
        BIGF[i, :] = f

        muC_s_t[i] = mu_Y + nu * (1 - bet) + (sigma_Y - Deltabar2[i]) * (
                part1[i] - Deltabar2[i])
...
for l in range(Mpaths):
    ...
    for k, That in enumerate(T_hats):
        ...
        for m, nu in enumerate(nus):
            ...
            (
                Deltabar, IntVec, Xt, Delta_s_t, Yt, Zt, f, tau
            ) = BuildUpCohortsMAIN(
            ...
            Zt = np.cumsum(dZt)
            (
                Xt2, Deltabar2, Part1, mu_S, mu_S_t, muhat_S_t, r_t, theta_t, Port, muC_s_t, sigmaC_s_t, BIGf, BIGDELTA,
                Et, Vt, dR
            ) = SimCohortsMAIN(
                biasvec, dZt, Nt, tau, IntVec, Delta_s_t, dt, rho, nu, Vbar, mu_Y, sigma_Y, sigma_S, bet, That, Npre
            )
