...
def build_cohorts_SI(
    ...
    X = np.ones((1, 1), dtype=np.float16)
    eta_st_eta_ss_init = np.ones((Ntype, 1), dtype=np.float16)
    ...
    tau_info = np.ones((Ntype, 1), dtype=np.float16) * dt
...
def build_cohorts_mix_type(
    ...
    # size of matrix: type * cohort; or type * 1; or 1 * cohort
    Delta_s_t = np.zeros((Ntype, Nconstraint, 1), dtype=np.float16)  # belief bias, eq(3)
    d_eta_st = np.zeros((Ntype, Nconstraint, 1), dtype=np.float16)  # disagreement, eq(11)
    X = np.ones((1, 1, 1))
    eta_st_eta_ss_init = np.ones((Ntype, Nconstraint, 1), dtype=np.int8)
    eta_st_eta_ss = eta_st_eta_ss_init
    ...
    tau_info = np.ones((Ntype, Nconstraint, 1), dtype=np.float16) * dt
    Vhat_init = np.ones((Ntype, Nconstraint, 1), dtype=np.float16) * Vhat
    Vhat_vector = Vhat_init
    a_phi = (1 - phi ** 2)
    phi_sqr_a_phi = phi / np.sqrt(a_phi)
    a_phi_1 = 1 / a_phi
