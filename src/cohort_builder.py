...
def build_cohorts_SI(
    ...
    invest_tracker = (
        np.ones((Ntype, Ninit), dtype=np.int8)
        if mode_trade != "complete"
        else np.ones((Ntype, Nc), dtype=np.int8)
    )
    ...
    for i in tqdm(range(1, Nc)):
        ...
        # Equation (15): belief updating rule, based on dZ and estimation error
        eta_st_eta_ss = eta_st_eta_ss * np.exp(
            (-0.5 * d_eta_st**2) * dt + d_eta_st * dZ_build_t
        )

        # Equation (20): rescaling X to ensure proper aggregation of consumption shares
        ...
        X_t = np.sum(X_parts) / (
            1 - tax * beta0 * dt
        )  # dividing by (1-tax*dt) keeps sum(f_st*dt) at 1
        ...
        # Belief updating using signal structure and prior/posterior variance (eqs. 6, 9)
        if mode_trade == "complete":
        ...
        if (
            mode_trade == "complete"
        ):  # where tau_info is the same with age; no switch between N and P for complete market
            tau_info = tau[:, -i - 1 :]
        ...
        # Newborn cohort belief initialization: either zero bias (too few observations) or Npre-based average bias
        if i < Npre:
            Delta_s_t += dDelta_s_t
            Delta_s_t = np.append(
                Delta_s_t, np.zeros((Ntype, 1)), axis=1
        ...
        if (
            i < Ninit or mode_trade == "complete"
        ):  # Ninit: initial rounds where the short-sale constraint is relaxed
            d_eta_st = Delta_s_t  # relax the short-sale constraint in the beginning

        elif mode_trade == "w_constraint":
            invest_tracker = np.append(
                invest_tracker, np.ones((Ntype, 1)), axis=1
            )  # indicator of current status, =1 for a cohort if type == P

            if (
                mode_learn == "disappointment"
            ):  # agents switch from type P to type N once constrained, and stay as type N
                ...
                Vhat_vector = np.append(
                    V_st_P, Vhat * np.ones((Ntype, 1)), axis=1
                ...
                # Solving for theta_t that clears the market (aggregate demand = supply), using bisection algorithm
                theta_t = bisection(
                    solve_theta,
                    lowest_bound,
                    50,
                ...
                Vhat_vector = (
                    np.append(V_st_P, Vhat * np.ones((Ntype, 1)), axis=1)
                    * switch_P_to_N
                    + np.append(V_st_N, Vhat * np.ones((Ntype, 1)), axis=1)
                    * switch_N_to_P
...
def build_cohorts_mix_type(
    ...
    for i in tqdm(range(1, Nc)):
        ...
        # Update signal ratio eta_st_eta_ss as in equation (15)
        eta_st_eta_ss = eta_st_eta_ss * np.exp(
            (-0.5 * d_eta_st**2) * dt + d_eta_st * dZ_build_t
        )
        ...
        if (
            ...
            invest_tracker = np.append(
                invest_tracker, invest_newborn, axis=2
            )  # indicator of current type, =1 for a cohort if type == P
            ...
            lowest_bound = -np.max(
                possible_delta_st[np.nonzero(possible_delta_st)]
            )  # absolute lower bound where no agent holds the stock
