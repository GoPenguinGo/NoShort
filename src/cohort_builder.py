...
def build_cohorts_SI(
    ...
    invest_tracker = np.ones((Ntype, Ninit), dtype=np.int8) if mode_trade != 'complete' else np.ones((Ntype, Nc))
    can_short_tracker = np.zeros((Ntype, Ninit), dtype=np.int8) if mode_trade == 'partial_constraint_rich' or mode_trade == 'partial_constraint_old' else np.zeros((Ntype, Nc))
