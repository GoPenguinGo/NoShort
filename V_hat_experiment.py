...
# mode_trade = 'keep'
mode_learn = 'give_up'
# mode_learn = 'back_collect'
# mode_learn = 'back_renew'
# zoom_in = 'small'
# zoom_in = 'large'
zoom_in = 'overall'
a = np.arange(1, 13, 1)
...
for l in range(Mpaths):
    ...
    for k, T_hat in enumerate(T_hats):
        ...
        if mode_trade in ['drop', 'keep', 'comp']:
            (
                r,
                theta,
                f,
                ...
                age_parti,
                n_parti,
            ) = simulate(mode_trade, mode_learn, Nc, Nt, dt, rho, nu, Vhat, mu_Y, sigma_Y, sigma_S, tax, beta, Npre,
                         Ninit, T_hat,
                         dZ_build, dZ, tau,
                         cohort_size)
            invest_tracker = pi > 0

        elif mode_trade in ['rich_free']:
            (
                r,
                theta,
                f,
                ...
                Delta_bar_long,
                Delta_bar_short,
            ) = simulate_partial_constraint(mode_trade, mode_learn, Nc, Nt, dt, rho, nu, Vhat, mu_Y, sigma_Y, sigma_S,
                                            tax, beta, Npre,
...
for i in range(len(ys)):
        ...
        plt.savefig('initial window and ' + xlabels[i] + '_' + mode_trade + '_' + mode_learn + '_' + zoom_in + '.png', dpi=200, format="png")
