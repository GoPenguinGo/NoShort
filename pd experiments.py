        ...
        for i in range(2):
            ...
            for j in range(2):
                ...
                if g <= 1:
                    ...
                    alpha_constraint = np.ones((1, Nconstraint)) * 1 / Nconstraint if g == 2 else np.ones((1, Nconstraint)) * (0.5, 0.5, 0, 0)
...
scenario_labels = ['Shocks', 'Complete', 'Reentry', 'Mix']
n_rows = 4
for jj in range(2):  # variable, sigma_S if jj == 0, pd if jj == 1
    fig, axes = plt.subplots(nrows=n_rows, ncols=1, sharex='all', figsize=(8, 10))
    y_mat = y_list[jj]
...
fig_title_list = ['Complete', 'Reentry']
# alternative_var_list = [r'$\rho=\{0.1\%, 0.5\%\}$',
#                         r'$\rho=\{0.1\%, 5\%\}$',
#                         r'$\rho=\{-1.5\%, 3.5\%\}$',]
alternative_var_list = [r'$\rho=\{0.1\%, 0.5\%\}$',
