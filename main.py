...
for j, ax in enumerate(axes):
    ...
    for i in range(n_phi_short):
        y = y_case[0, i, :N_cut]
        label_i = 'phi=' + str('{0:.2f}'.format(phi_vector_short[i]))
        ax.plot(x[data_point], y[data_point], color=colors_short[i], linewidth=0.5, label=label_i)
    if j == 0:
        ax.set_title(r'Average $\mid\Delta_{s,t}\mid$', color='black')
        ax.legend()
    else:
        ax.set_title('Average participation probability', color='black')
    ax.tick_params(axis='y', labelcolor='black')
fig.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig(str(N_1) + 'paths, ' + str(t_cut) + 'age, ' +'Average estimation error and age.png', dpi=300)
plt.show()
#plt.close()
...
red_case = 1
yellow_case = 1
titles_subfig = [r'Aggregate wealth share of participants $\bar{\Delta}_t$', r'Wealth weighted average estimation error conditional on participation $\Phi_t$', 'Participation rate']
...
titles_subfig = [r'Portfolios, across scenarios, $\phi=0.4$', r'Portfolios, reentry, across $\phi$ values', r'Belief component, across scenarios, $\phi=0.4$', r'Consumption component, across scenarios, $\phi=0.4$']
